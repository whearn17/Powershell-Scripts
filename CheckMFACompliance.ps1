...
$users = Get-ADUser -Filter * -SearchBase $ou -SearchScope $searchScope -Properties MemberOf | Sort-Object Name 

# Check if a user is missing MFA
function CheckIfUserIsMissingMFA($user) {
...
$missingMFAUsers = $users | Where-Object { CheckIfUserIsMissingMFA $_ }
